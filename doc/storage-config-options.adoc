...
Archiving a PVC retains its data when `delete` is called on it. This option can be enabled by specifying `pvReclaimPolicy` either through `StorageClass` or `Kadalu CLI`. The `pvReclaimPolicy` takes either `delete` or `archive`. Default value is `delete` which deletes the PVC along with its data. `archive` option retains the data by renaming `pvc-123` to `archived-pvc-123`.
