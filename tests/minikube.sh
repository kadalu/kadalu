...
kadalu_operator)
    ...
    sleep 3
    # Start storage
    wget -O kadalu-storage.yaml https://raw.githubusercontent.com/amarts/kadalu/master/examples/sample-storage-file-device.yaml
    sed -i -e "s/DISK/${DISK}/g" kadalu-storage.yaml
    kubectl create -f kadalu-storage.yaml
    ;;
test_kadalu)
    # TODO: get the proper file
    wget -O kadalu-test-app.yaml https://raw.githubusercontent.com/amarts/kadalu/master/examples/sample-test-app.yaml
    # Run ReadWriteOnce test
    kubectl create -f kadalu-test-app.yaml
    sleep 4
    kubectl get pods pod1
    kubectl logs pod1

    sed -i -e "s/pv1/pv2/g" kadalu-test-app.yaml
    sed -i -e "s/pod1/pod2/g" kadalu-test-app.yaml
    sed -i -e "s/ReadWriteOnce/ReadWriteMany/g" kadalu-test-app.yaml
    kubectl create -f kadalu-test-app.yaml
    sleep 4
