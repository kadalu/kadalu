...
def update_block_volume(pvtype, pool_mnt, volname, expansion_requested_pvsize):
...
def delete_volume(volname):
    ...
    hash_start = len(os.path.join(POOL_MOUNTDIR, vol.pool, vol.voltype, ''))
    end = len(volpath)
    ...
    hash_start = len(os.path.join(POOL_MOUNTDIR, vol.pool, "info", vol.voltype, ''))
...
def search_volume(volname):
    ...
    for pool in pools:
        pool_name = pool['name']
        mntdir = os.path.join(POOL_MOUNTDIR, pool_name)
        mount_glusterfs(pool, mntdir)
